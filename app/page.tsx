...
export default function Home() {
  ...
  useEffect(() => {
    if (showIntro) return // イントロ中はアニメーションを開始しない
      ...
      <main className={`min-h-screen bg-black transition-opacity duration-1000 ease-in-out ${showIntro ? "opacity-0" : "opacity-100"}`}>
        ...
        <section className="relative h-screen flex flex-col items-center justify-center luxury-bg overflow-hidden pt-20">
          ...
          <div className="container mx-auto px-4 text-center z-30 relative">
            <h1 className={`single-line-title gold-gradient-text transition-all duration-1000 delay-500 ${!showIntro ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
              AirFuture MINI
            </h1>
