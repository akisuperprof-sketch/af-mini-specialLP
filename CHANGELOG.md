...
## [v1.1.0] - 2026-02-02
### Changed
- **動画パーティクルの演出強化**: 静止していた粒子に上昇感と揺らぎのアニメーションを追加。
- **動画のモバイル最適化**: 垂直画面でもテキストが切れないよう、セーフゾーンを意識したレイアウト調整と文字サイズの適正化。
- **LPレスポンシブ改善**: SP版でのタイトル突き抜けを修正。`sm` ブレイクポイントでの改行制御を導入。
- **ボタン演出の磨き込み**: ゴールドボタンのシャドウとグラデーションの調整。
- **保守性の向上**: `.gitignore` の最適化。AIエージェント関連ディレクトリおよびビルド成果物の除外設定を強化。
